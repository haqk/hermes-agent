...
DELEGATE_TASK_SCHEMA = {
    "name": "delegate_task",
    # Original description before compression — see git history for full version
    "description": (
        "Spawn subagents for tasks in isolated contexts. Each gets own conversation, "
        "terminal session & toolset. Only final summary returned — intermediate results "
        "never enter your context.\n\n"
        "TWO MODES (one of 'goal' | 'tasks' required):\n"
        "1. Single: provide 'goal' (+ optional context, toolsets)\n"
        "2. Batch (parallel): provide 'tasks' array, up to 3. "
        "All run concurrently, results returned together.\n\n"
        "WHEN TO USE:\n"
        "- Reasoning-heavy subtasks (debugging, code review, research synthesis)\n"
        "- Tasks that would flood context w/ intermediate data\n"
        "- Parallel independent workstreams\n\n"
        "WHEN NOT TO USE:\n"
        "- Mechanical multi-step w/ no reasoning → execute_code\n"
        "- Single tool call → call directly\n"
        "- Needs user interaction → subagents cannot use clarify\n\n"
        "IMPORTANT:\n"
        "- Subagents have NO memory of your conversation. Pass all info "
        "(paths, errors, constraints) via 'context' field.\n"
        "- Subagents CANNOT call: delegate_task, clarify, memory, send_message, "
        "execute_code.\n"
        "- Each subagent gets own terminal session (separate workdir & state).\n"
        "- Results always returned as array, one entry per task."
    ),
    "parameters": {
        "type": "object",
        "properties": {
